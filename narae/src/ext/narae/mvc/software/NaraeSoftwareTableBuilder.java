package ext.narae.mvc.software;
...
@ComponentBuilder("ext.narae.document.SoftwareList")
public class NaraeSoftwareTableBuilder extends AbstractComponentBuilder {
	...
	public Object buildComponentData(ComponentConfig config, ComponentParams params) throws Exception {
		...
		if (creator != null && creator.trim().length() > 0) {
			WTPrincipal principal = (WTPrincipal) OrganizationServicesHelper.manager.getPrincipal(creator);
			if (principal != null) {
				if (query.getConditionCount() > 0)
					query.appendAnd();
				ClassAttribute classAttribute = new ClassAttribute(WTDocument.class, "creator.key.id");
		...
		if (query.getConditionCount() > 0) {
			...
			// Except SW Document
			query.appendAnd();
			query.appendWhere(new SearchCondition(WTDocument.class, "docType", SearchCondition.EQUAL, "$$SWDocument"));
	...
	public ComponentConfig buildComponentConfig(ComponentParams arg0) throws WTException {
		...
		// "DOC_MNG_SEARCH_DOC_TITLE", new Object[]{},
