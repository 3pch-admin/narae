	...
	public Object buildComponentData(ComponentConfig config, ComponentParams params) throws Exception {
		...
		if (state != null) {
			...
			if (state != null && !state.trim().equals("ECR_PICKER_SEARCH")
					&& !state.trim().equals("ECO_PICKER_SEARCH")) {
				...
				if (state != null && state.trim().equals("ECR_PICKER_SEARCH")) {
					query.appendOpenParen();
					query.appendWhere(
							new SearchCondition(WTPart.class, "state.state", SearchCondition.EQUAL, "APPROVED"),
							new int[] { idx });
					query.appendOr();
					query.appendWhere(
							new SearchCondition(WTPart.class, "state.state", SearchCondition.EQUAL, "SENT_ERP"),
							new int[] { idx });
					query.appendOr();
					query.appendWhere(new SearchCondition(WTPart.class, "state.state", SearchCondition.EQUAL,
							"PUR_ORDER_APPROVAL"), new int[] { idx });
					query.appendCloseParen();
				} else if (state != null && state.trim().equals("ECO_PICKER_SEARCH")) {
					query.appendOpenParen();
					query.appendWhere(new SearchCondition(WTPart.class, "state.state", SearchCondition.EQUAL, "INWORK"),
							new int[] { idx });
					query.appendOr();
					query.appendWhere(new SearchCondition(WTPart.class, "state.state", SearchCondition.EQUAL, "RETURN"),
							new int[] { idx });
					query.appendCloseParen();
				}
			}
