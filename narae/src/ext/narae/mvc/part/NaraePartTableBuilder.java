...
@ComponentBuilder("ext.narae.part.PartList")
public class NaraePartTableBuilder extends AbstractComponentBuilder {
	...
	public Object buildComponentData(ComponentConfig config, ComponentParams params)
		...
		String selectedFolderFromFolderContext = (String) params.getParameter("selectedFolderFromFolderContext");
		...
		// Default c/i c/o
		if ( state != null && (state.trim().equals("ECR_PICKER_SEARCH") || state.trim().equals("ECO_PICKER_SEARCH")) ) {
			query.appendWhere(new SearchCondition(WTPart.class, "checkoutInfo.state",
					SearchCondition.EQUAL, "c/i"));
		} else {
		...
		if(state != null ) {
			...
			if( state != null && !state.trim().equals("ECR_PICKER_SEARCH") && !state.trim().equals("ECO_PICKER_SEARCH") ) {
				...
				if( state != null && state.trim().equals("ECR_PICKER_SEARCH") ) {
					query.appendOpenParen();
					query.appendWhere(new SearchCondition(WTPart.class, "state.state", SearchCondition.EQUAL, "APPROVED"), new int[]{idx});
					query.appendOr();
					query.appendWhere(new SearchCondition(WTPart.class, "state.state", SearchCondition.EQUAL, "SENT_ERP"), new int[]{idx});
					query.appendOr();
					query.appendWhere(new SearchCondition(WTPart.class, "state.state", SearchCondition.EQUAL, "PUR_ORDER_APPROVAL"), new int[]{idx});
					query.appendCloseParen();
				} else if ( state != null && state.trim().equals("ECO_PICKER_SEARCH") ) {
					query.appendOpenParen();
					query.appendWhere(new SearchCondition(WTPart.class, "state.state", SearchCondition.EQUAL, "INWORK"), new int[]{idx});
					query.appendOr();
					query.appendWhere(new SearchCondition(WTPart.class, "state.state", SearchCondition.EQUAL, "RETURN"), new int[]{idx});
					query.appendCloseParen();
				}
			}
		}
		...
		if(desc != null && desc.trim().length() > 0) {
	        ...
	        if (aview != null) {
	            ...
	            query.appendWhere(new SearchCondition(StringValue.class, "definitionReference.hierarchyID",
	                              ...
	                              new int[] { _idx });
	            query.appendAnd();
	            query.appendWhere(new SearchCondition(StringValue.class, "value", SearchCondition.LIKE, ("%"+desc.trim()+"%").toUpperCase()),
	                    new int[] { _idx });
	        }
		}
		...
		if(spec != null && spec.trim().length() > 0){
		        ...
		        if (aview != null) {
		            ...
		            query.appendWhere(new SearchCondition(StringValue.class, "definitionReference.hierarchyID",
		                              ...
		                              new int[] { _idx });
		            query.appendAnd();
		            query.appendWhere(new SearchCondition(StringValue.class, "value", SearchCondition.LIKE, ("%"+spec.trim()+"%").toUpperCase()),
		                    new int[] { _idx });
		        }
		} else spec = "";
		...
		if(maker != null && maker.trim().length() > 0){
		        ...
		        if (aview != null) {
		            ...
		            query.appendWhere(new SearchCondition(StringValue.class, "definitionReference.hierarchyID",
		                              ...
		                              new int[] { _idx });
		            query.appendAnd();
		            query.appendWhere(new SearchCondition(StringValue.class, "value", SearchCondition.LIKE, ("%"+maker.trim()+"%").toUpperCase()),
		                    new int[] { _idx });
		        }
		} else maker = "";
...
//			WTUser user = people.getUser()
			...
			if(query.getConditionCount()>0)query.appendAnd();
			query.appendWhere(new SearchCondition(WTPart.class,"iterationInfo.creator.key","=", PersistenceHelper.getObjectIdentifier( user )), new int[]{idx});
		}
...
//			return result;
	...
	public ComponentConfig buildComponentConfig(ComponentParams arg0)
        ...
        table.setLabel(WTMessage.getLocalizedMessage(RESOURCE , "SEARCH_PART", new Object[]{}, WTContext.getContext().getLocale()));
        ...
        numberCol.setLabel(WTMessage.getLocalizedMessage(RESOURCE , "PART_NUMBER", new Object[]{}, WTContext.getContext().getLocale()));
        ...
        nameCol.setLabel(WTMessage.getLocalizedMessage(RESOURCE , "PART_NAME", new Object[]{}, WTContext.getContext().getLocale()));
...
//        table.addComponent(factory.newColumnConfig(CHANGE_STATUS_FAMILY, false));
        ...
        // State column is a DataStore-only column (please check in the UI whether this column is listed)
        

        // strikeThrough column handling : "strikeThroughRow" should be the ID as the default DataUtility is mapped to
        // this
...
//        table.addComponent(factory.newColumnConfig(VERSION, true));
        ...
        ColumnConfig specCol = factory.newColumnConfig("Spec", true);
        table.addComponent(specCol);
        
        ColumnConfig makerCol = factory.newColumnConfig("Maker", true);
        table.addComponent(makerCol);
