...
//	private static Logger log = Logger.getLogger(NaraeDrawingTableBuilder.class.getName());
	...
	public Object buildComponentData(ComponentConfig config, ComponentParams params)
		...
		String partFolder = params.getParameter("selectedFolderFromFolderContext")!=null?(String) params.getParameter("selectedFolderFromFolderContext"):"";
        ...
        String islastversion = params.getParameter("islastversion")!=null?(String) params.getParameter("islastversion"):"";
        ...
        String authoringType = params.getParameter("authoringType")!=null?(String) params.getParameter("authoringType"):"";
        ...
        if( first.trim().equals("true") ) return new QueryResult();
		...
		query.appendWhere(new SearchCondition(EPMDocumentMaster.class, "thePersistInfo.theObjectIdentifier.id",
                EPMDocument.class, "masterReference.key.id"),
                new int[] { idx2, idx });
		...
		query.appendWhere(new SearchCondition(EPMDocument.class, "checkoutInfo.state",
				SearchCondition.EQUAL, "c/i"), new int[] { idx });
		query.appendOr();
		query.appendWhere(new SearchCondition(EPMDocument.class, "checkoutInfo.state",
				SearchCondition.EQUAL, "c/o"), new int[] { idx });
		...
		//		folders.add(folder);
			...
			query.appendWhere(new SearchCondition(IteratedFolderMemberLink.class, 
										...
										folder.getPersistInfo().getObjectIdentifier().getId()), new int[]{folder_idx});
...
//		}
			...
			for(int fi = 0; fi < folders.size(); fi++) {
				...
				query.appendWhere(new SearchCondition(EPMDocument.class, EPMDocument.DOC_TYPE, SearchCondition.EQUAL, docType), new int[] { idx
				});
				}
...
//		}
	    ...
	    if(material != null && material.trim().length() > 0){
	        ...
	        if (aview != null) {
	            ...
	            query.appendWhere(new SearchCondition(StringValue.class, "definitionReference.hierarchyID",
	                              ...
	                              new int[] { idx3 });
	            query.appendAnd();
	            query.appendWhere(new SearchCondition(StringValue.class, "value", SearchCondition.LIKE, ("%"+material.trim()+"%").toUpperCase()),
	                    new int[] { idx3 });
	            query.appendCloseParen();
	        }
	    }else material = "";
	    ...
	    if(treatment != null && treatment.trim().length() > 0){
	        ...
	        if (aview != null) {
	            ...
	            query.appendWhere(new SearchCondition(StringValue.class, "definitionReference.hierarchyID",
	                              ...
	                              new int[] { idx3 });
	            query.appendAnd();
	            query.appendWhere(new SearchCondition(StringValue.class, "value", SearchCondition.LIKE, ("%"+treatment.trim()+"%").toUpperCase()),
	                    new int[] { idx3 });
	            query.appendCloseParen();
	        }
	    }else treatment = "";
	    ...
	    if(drawingType != null && drawingType.trim().length() > 0){
	        ...
	        if (aview != null) {
	            ...
	            query.appendWhere(new SearchCondition(StringValue.class, "definitionReference.hierarchyID",
	                              ...
	                              new int[] { idx3 });
	            query.appendAnd();
	            query.appendWhere(new SearchCondition(StringValue.class, "value", SearchCondition.LIKE, ("%"+drawingType.trim()+"%").toUpperCase()),
	                    new int[] { idx3 });
	            query.appendCloseParen();
	        }
	    }else drawingType = "";
		...
		if( detailFlag.equals("true") ) {
			...
			// Date
			if( (predate != null && predate.trim().length() > 0) || (postdate != null && postdate.trim().length() > 0) ) {
				if( query.getConditionCount() > 0) query.appendAnd();
				
				if( (predate != null && predate.trim().length() > 0) && (postdate == null || postdate.trim().length() == 0) ) {
					query.appendWhere(
							new SearchCondition(EPMDocument.class, 
									"thePersistInfo.createStamp", 
									SearchCondition.GREATER_THAN_OR_EQUAL, 
									Timestamp.valueOf(predate + " 00:00:00"))
							, new int[]{idx});
				} else if ( (predate == null || predate.trim().length() == 0) && (postdate != null && postdate.trim().length() > 0) ) {
					query.appendWhere(
							new SearchCondition(EPMDocument.class, 
									"thePersistInfo.createStamp", 
									SearchCondition.LESS_THAN_OR_EQUAL, 
									Timestamp.valueOf(postdate + " 23:59:59"))
							, new int[]{idx});
				} else {
					query.appendOpenParen();
					query.appendWhere(
...
//				WTUser user = people.getUser();
	...
	public ComponentConfig buildComponentConfig(ComponentParams arg0)
        ...
        //table.setLabel(WTMessage.getLocalizedMessage(RESOURCE , "SEARCH_DRAWING", new Object[]{}, WTContext.getContext().getLocale()));
        ...
        numberCol.setLabel(WTMessage.getLocalizedMessage(RESOURCE , "DRAWING_NUMBER", new Object[]{}, WTContext.getContext().getLocale()));
        ...
        nameCol.setLabel(WTMessage.getLocalizedMessage(RESOURCE , "DRAWING_NAME", new Object[]{}, WTContext.getContext().getLocale()));
