...
//	        }
			...
			if (aview != null) {
				...
				int _idx = query.appendClassList(StringValue.class, false);
				query.appendWhere(new SearchCondition(StringValue.class, "theIBAHolderReference.key.id",
						WTChangeRequest2.class, "thePersistInfo.theObjectIdentifier.id"), new int[] { _idx, idx });
				query.appendAnd();
				query.appendWhere(new SearchCondition(StringValue.class, "definitionReference.hierarchyID",
				...
				query.appendWhere(new SearchCondition(StringValue.class, "value", SearchCondition.LIKE,
						("%" + project.trim() + "%").toUpperCase()), new int[] { _idx });
			}
		}
		...
		if (tempUserList.size() > 0) {
			...
			for (int i = 0; i < tempUserList.size(); i++) {
				...
				ClassAttribute classAttribute = new ClassAttribute(WTChangeRequest2.class,
						"iterationInfo.creator.key.id");
...
//		if (query.getConditionCount() > 1) {
			...
			if (query.getConditionCount() > 0)
				query.appendAnd();
			query.appendWhere(
					new SearchCondition(WTChangeRequest2.class, "state.state", SearchCondition.NOT_EQUAL, "INWORK"));
			LatestConfigSpec latestCSpec = new LatestConfigSpec();
			QueryResult result = ConfigHelper.service.queryIterations(query, null);
			System.out.println("request=" + result.size());
			return result;
//		} else {
...
//		}
	...
	public ComponentConfig buildComponentConfig(ComponentParams arg0) throws WTException {
		...
		table.setLabel(WTMessage.getLocalizedMessage(RESOURCE, "ECR_SEARCH", new Object[] {},
		...
		numberCol.setLabel(WTMessage.getLocalizedMessage(RESOURCE, "ECR_NUMBER", new Object[] {},
				WTContext.getContext().getLocale()));
		...
		nameCol.setLabel(WTMessage.getLocalizedMessage(RESOURCE, "ECR_NAME", new Object[] {},
				WTContext.getContext().getLocale()));
		...
		supporterCol.setLabel(WTMessage.getLocalizedMessage(RESOURCE, "ECR_SUPPORTER", new Object[] {},
				WTContext.getContext().getLocale()));
		table.addComponent(supporterCol);

		ColumnConfig ecoCol = factory.newColumnConfig("Related_ECO", true);
		ecoCol.setLabel(WTMessage.getLocalizedMessage(RESOURCE, "ECR_RELATED_ECO", new Object[] {},
				WTContext.getContext().getLocale()));
		...
		creatorCol.setLabel(WTMessage.getLocalizedMessage(RESOURCE, "ECR_REQUESTOR", new Object[] {},
				WTContext.getContext().getLocale()));
		...
		reasonCol.setLabel(WTMessage.getLocalizedMessage(RESOURCE, "ECR_REASON", new Object[] {},
				WTContext.getContext().getLocale()));
