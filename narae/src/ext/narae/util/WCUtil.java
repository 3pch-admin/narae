...
public class WCUtil {
	...
	public static WTPrincipalReference getWTPrincipalReference(String id) {
		...
		try {
			wtpr = OrganizationServicesHelper.manager.getPrincipalReference(id,
	...
	public static WTLibrary getLibrary() throws Exception {
		QuerySpec query = new QuerySpec();
		int idx = query.appendClassList(WTLibrary.class, true);
		SearchCondition sc = new SearchCondition(WTLibrary.class, WTLibrary.NAME, "=", "LIB");
		query.appendWhere(sc, new int[] { idx });
		QueryResult result = PersistenceHelper.manager.find(query);
		while (result.hasMoreElements()) {
			Object[] obj = (Object[]) result.nextElement();
			WTLibrary library = (WTLibrary) obj[0];
			return library;
		}
		return null;
	}
	...
	public static PDMLinkProduct getPDMLinkProduct() throws Exception {
		...
		SearchCondition sc1 = new SearchCondition(PDMLinkProduct.class, PDMLinkProduct.NAME, SearchCondition.EQUAL,
				productName);
	...
	public static PDMLinkProduct getPDMLinkProductForDrawing() throws Exception {
		...
		QuerySpec qs = new QuerySpec(PDMLinkProduct.class);
		SearchCondition sc1 = new SearchCondition(PDMLinkProduct.class, "containerInfo.name", "=", productName);
		qs.appendSearchCondition(sc1);
	...
	public static String getOIDString(Persistable per) {
		try {
			/*
			 * ReferenceFactory rf = new ReferenceFactory(); return
			 * rf.getQueryString(rf.getReference(per));
			 */
			if (per == null) {
				return "";
			}
