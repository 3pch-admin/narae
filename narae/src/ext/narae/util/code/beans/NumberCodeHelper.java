...
public class NumberCodeHelper
{
    ...
    public NumberCode2 getNumberCode2(String codeType, String code,boolean isTop)
    {
        ...
        {	
            ...
            if(isTop){
            	select.appendAnd();
            	select.appendWhere(new SearchCondition(NumberCode2.class, "parentReference.key.id", "=", longOid), new int[] { 0 });
            }
            QueryResult result = PersistenceHelper.manager.find(select);
            while (result.hasMoreElements())
            {
    ...
    public NumberCode2 getNumberCode2(String codeType, String code,NumberCode2 parent)
    {
        ...
        {	
            ...
            if(null!=parent){
            	select.appendAnd();
            	select.appendWhere(new SearchCondition(NumberCode2.class, "parentReference.key.id", "=", CommonUtil.getOIDLongValue(parent)), new int[] { 0 });
            }
            QueryResult result = PersistenceHelper.manager.find(select);
            while (result.hasMoreElements())
            {
		...
		try {
			...
			NumberCode child = null;
	  		...
	  		//for(int i = 0; i < r_list.size(); i++) {
	  			...
	  			//System.out.println((String)objArr[0] + '\t' + (String)objArr[1] + '\t' + (String)objArr[2]);
	  		//}
	  		//System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
	...
	public static QuerySpec getCodeQuerySpec2(HashMap map) throws WTException {
		...
		try {
			...
			if(type != null && type.length() > 0) {
				sc = new SearchCondition(NumberCode2.class, "codeType", SearchCondition.EQUAL, NumberCodeType.toNumberCodeType(type));
			...
			if(description != null && description.length() > 0) {
				...
				sc = new SearchCondition(NumberCode2.class, "description", SearchCondition.LIKE, "%"+description+"%");
				qs.appendWhere(sc, new int[] { i });
			}
			...
			if(isParent != null && "false".equals(isParent.toLowerCase())) {
				...
				if(parent != null) {
					...
					sc = new SearchCondition(NumberCode2.class, "parentReference.key.id", 
...
/*******************************************************************************
 * $Log: NumberCodeHelper.java,v $
 * Revision 1.3  2011/06/12 03:51:04  tsuam
 *
 ...
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.1  2011/04/21 03:50:59  thhwang
 * *** empty log message ***
 *
 * Revision 1.1  2011/03/08 01:50:03  thhwang
 * 占쏙옙占?
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 * http://march-hare.com/cvsnt/
 ...
 * http://march-hare.com/cvsnt/
 *
 * Revision 1.1.1.1  2009/07/01 07:07:43  administrator
 * no message
 *
 * Committed on the Free edition of March Hare Software CVSNT Server.
 * Upgrade to CVS Suite for more features and support:
 ...
 * 占쌍쇽옙 占썩본 占쏙옙키占쏙옙 占쏙옙占쏙옙占쌜억옙 占싹뤄옙
 *
 * Revision 1.1  2008/01/23 09:51:53  sjhan
 * e3ps package 占쏙옙占쏙옙 占싹료본
 * jsp 占쌀쏙옙占쏙옙 확占쏙옙 占쏙옙 占쏙옙占쏙옙占쌔억옙 占쏙옙 占십울옙 占쏙옙占쏙옙
 *
