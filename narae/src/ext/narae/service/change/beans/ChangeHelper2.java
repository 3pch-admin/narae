...
public class ChangeHelper2 implements Serializable, RemoteAccess {
	...
	public static final ChangeHelper2 manager = new ChangeHelper2();
	...
	public static WTChangeRequest2 createECR(HashMap<String, Object> hash, boolean submitFlag) throws Exception {
		if (!RemoteMethodServer.ServerFlag) {
			...
			try {
				return (WTChangeRequest2) wt.method.RemoteMethodServer.getDefault().invoke("createECR",
						ChangeHelper2.class.getName(), null, argTypes, args);
...
//           TypeHelper.setType(change, objType);
			...
			String name = ((String) hash.get("name") != null) ? URLDecoder.decode((String) hash.get("name"), "utf-8")
					: "";
			String requestType = ((String) hash.get("purpose") != null)
					? URLDecoder.decode((String) hash.get("purpose"), "utf-8")
					: "";
			String requestor = ((String) hash.get("requestor") != null)
					? URLDecoder.decode((String) hash.get("requestor"), "utf-8")
					: "";
			String supportor = ((String) hash.get("supporter") != null)
					? URLDecoder.decode((String) hash.get("supporter"), "utf-8")
					: "";
			String projectName = ((String) hash.get("projectName") != null)
					? URLDecoder.decode((String) hash.get("projectName"), "utf-8")
					: "";
			String problem = ((String) hash.get("problem") != null)
					? URLDecoder.decode((String) hash.get("problem"), "utf-8")
					: "";
			String solution = ((String) hash.get("solution") != null)
					? URLDecoder.decode((String) hash.get("solution"), "utf-8")
					: "";
			String projectNumber = ((String) hash.get("prjNo") != null) ? (String) hash.get("partListOid") : "";
			String partOid = ((String) hash.get("partListOid") != null)
					? URLDecoder.decode((String) hash.get("partListOid"), "utf-8")
					: "";
			...
			DomainAdministeredHelper.setAdminDomain(team,
					((WTContainer) containerRef.getObject()).getSystemDomainReference());
...
//	           WTUser workerUser = workerPeople.getUser();
				...
				makeRoleMap(team, "SUPPORT ENGINEER",
						workerUser.getPersistInfo().getObjectIdentifier().getStringValue());
			}

			change = (WTChangeRequest2) TeamHelper.setTeam(change, team);

			// Set LC
			LifeCycleHelper.setLifeCycle(change,
					LifeCycleHelper.service.getLifeCycleTemplate("Narae_ECM_LC", containerRef)); // Lifecycle
	...
	public static WTChangeRequest2 updateECR(HashMap<String, Object> hash, boolean submitFlag) throws Exception {
		...
		try {
			...
			if (!name.equals(targetObject.getName())) {
				...
				WTChangeRequest2MasterIdentity identity = (WTChangeRequest2MasterIdentity) master
	...
	public static WTChangeOrder2 createECO(HashMap<String, Object> hash, boolean submitFlag) throws Exception {
		/****
		 * TEST **** hash = new HashMap<String,Object>(); hash.put("name",
		 * URLEncoder.encode("Test001", "UTF-8")); hash.put("changeType",
		 * URLEncoder.encode("A,B", "UTF-8")); hash.put("projectName",
		 * URLEncoder.encode("123456", "UTF-8")); hash.put("problem",
		 * URLEncoder.encode("AAAAAAAAA", "UTF-8")); hash.put("solution",
		 * URLEncoder.encode("BBBBBBBB", "UTF-8")); hash.put("relatedECR",
		 * URLEncoder.encode("", "UTF-8")); hash.put("beforeOid",
		 * URLEncoder.encode("VR:wt.part.WTPart:191526,VR:wt.part.WTPart:214028",
		 * "UTF-8")); hash.put("afterOid",
		 * URLEncoder.encode("VR:wt.part.WTPart:191526,VR:wt.part.WTPart:214028",
		 * "UTF-8")); TEST
		 ****/
		...
		if (!RemoteMethodServer.ServerFlag) {
			...
			Object args[] = new Object[] { hash, submitFlag };
			try {
				return (WTChangeOrder2) wt.method.RemoteMethodServer.getDefault().invoke("createECO",
						ChangeHelper2.class.getName(), null, argTypes, args);
			} catch (RemoteException e) {
				e.printStackTrace();
				throw new WTException(e);
...
//			WTContainerRef containerRef = WCUtil.getWTContainerRefForPart();
			...
			String projectName = ((String) hash.get("projectName") != null)
					? URLDecoder.decode((String) hash.get("projectName"), "utf-8")
					: "";
			String reason = ((String) hash.get("reason") != null)
					? URLDecoder.decode((String) hash.get("reason"), "utf-8")
					: " ";
			String measures = ((String) hash.get("measures") != null)
					? URLDecoder.decode((String) hash.get("measures"), "utf-8")
					: "";
			...
			// Set IBA
			AttributeHelper.service.setValue(change, iba);
			log.debug("8. Complete set IBA [EC_Reason,Project,description2]:" + purpose + "," + projectName + ","
					+ measures);
			...
			LifeCycleHelper.setLifeCycle(activity,
					LifeCycleHelper.service.getLifeCycleTemplate("Narae_ECM_LC", containerRef)); // Lifecycle
	...
	public static WTChangeOrder2 updateECO(HashMap<String, Object> hash, boolean submitFlag) throws Exception {
		...
		if (!RemoteMethodServer.ServerFlag) {
			...
			Object args[] = new Object[] { hash, submitFlag };
			try {
				return (WTChangeOrder2) wt.method.RemoteMethodServer.getDefault().invoke("updateECO",
						ChangeHelper2.class.getName(), null, argTypes, args);
			} catch (RemoteException e) {
				e.printStackTrace();
				throw new WTException(e);
	...
	public static String submitChangeObject(String oid) {
		if (!RemoteMethodServer.ServerFlag) {
			...
			try {
				return (String) wt.method.RemoteMethodServer.getDefault().invoke("submitChangeObject",
						ChangeHelper2.class.getName(), null, argTypes, args);
			} catch (Exception e) {
				e.printStackTrace();
				return e.getMessage();
	...
	public static String withdrawChangeObject(String oid) {
		if (!RemoteMethodServer.ServerFlag) {
			...
			Object args[] = new Object[] { oid };
			try {
				return (String) wt.method.RemoteMethodServer.getDefault().invoke("withdrawChangeObject",
						ChangeHelper2.class.getName(), null, argTypes, args);
			} catch (Exception e) {
				e.printStackTrace();
				return e.getMessage();
		...
		try {
			...
			Enumeration process = WfEngineHelper.service.getAssociatedProcesses(change, WfState.OPEN,
	...
	private static String clearAllProcess(String oid) {
		if (!RemoteMethodServer.ServerFlag) {
			...
			Object args[] = new Object[] { oid };
			try {
				return (String) wt.method.RemoteMethodServer.getDefault().invoke("clearAllProcess",
						ChangeHelper2.class.getName(), null, argTypes, args);
			} catch (Exception e) {
				e.printStackTrace();
				return e.getMessage();
		...
		try {
			...
			WTContained change = (WTContained) CommonUtil2.getInstance(oid);
			// Get WfPRocess & Delete WfProcess
			Enumeration process = WfEngineHelper.service.getAssociatedProcesses(change,
					WfState.CLOSED_COMPLETED_EXECUTED, change.getContainerReference());
			if (process != null) {
				WfProcess oneProcess = null;
				while (process.hasMoreElements()) {
					oneProcess = (WfProcess) process.nextElement();
	...
	private static void clearRelevantData(WTChangeRequest2 request) throws WTException {
		...
		SearchCondition condition = new SearchCondition(RelevantRequestData2.class, "roleAObjectRef.key.branchId",
	...
	private static void clearRelevantECR(WTChangeOrder2 order) throws WTException {
		...
		SearchCondition condition = new SearchCondition(AddressedBy2.class, "roleBObjectRef.key.branchId",
				SearchCondition.EQUAL, modelId);
		spec.appendWhere(condition);
	...
	private static void clearBeforePart(WTChangeActivity2 activity) throws WTException {
		...
		SearchCondition condition = new SearchCondition(AffectedActivityData.class, "roleAObjectRef.key.branchId",
				SearchCondition.EQUAL, modelId);
		spec.appendWhere(condition);
	...
	private static void clearAfterPart(WTChangeActivity2 activity) throws WTException {
		...
		SearchCondition condition = new SearchCondition(ChangeRecord2.class, "roleAObjectRef.key.branchId",
				SearchCondition.EQUAL, modelId);
		spec.appendWhere(condition);
	...
	private static void makeRoleMap(Team team, String roleName, String roler)
		...
		if (partOidList.length > 0) {
			...
			Role role = (Role) (EnumeratedTypeUtil
					.toEnumeratedType(EnumeratedTypeUtil.getStringValue(Role.class.getName(), roleName)));
	...
	public static WfProcess startProcess(WTObject pbo, String workflowTemplateName, String processNamePrefix)
	...
	public String getNextNumber(String seqName) throws Exception {
		DecimalFormat df = new DecimalFormat("00000");
		QuerySpec query = new QuerySpec();
		int idx = query.appendClassList(WTChangeOrder2Master.class, true);

		SearchCondition sc = new SearchCondition(WTChangeOrder2Master.class, WTChangeOrder2Master.NUMBER, "LIKE",
				seqName + "%");
		query.appendWhere(sc, new int[] { idx });

		ClassAttribute ca = new ClassAttribute(WTChangeOrder2Master.class, WTChangeOrder2Master.NUMBER);
		...
		if (result.hasMoreElements()) {
			...
			WTChangeOrder2Master master = (WTChangeOrder2Master) obj[0];
