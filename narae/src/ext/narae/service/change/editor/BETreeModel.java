...
public class BETreeModel extends DefaultTreeModel {
	...
	/**
	 * Determines how the <code>isLeaf</code> method figures out if a node is a leaf
	 * node. If true, a node is a leaf node if it does not allow children. (If it
	 * allows children, it is not a leaf node, even if no children are present.)
	 * That lets you distinguish between <i>folder</i> nodes and <i>file</i> nodes
	 * in a file system, for example.
	 * <p>
	 * If this value is false, then any node which has no children is a leaf node,
	 * and any node may acquire children.
	 *
	 ...
	 *                           true if each node is asked to see if it can have
	 *                           children
	 ...
	 *
	 * @return true if only nodes which do not allow children are leaf nodes, false
	 *         if nodes which have no children (even if allowed) are leaf nodes
	 * @see #asksAllowsChildren
	 */
	...
	/**
	 * Sets the root to <code>root</code>. A null <code>root</code> implies the tree
	 * is to display nothing, and is legal.
	 */
	 ...
	 * leaf or if it has no children. <I>parent</I> must be a node previously
	 ...
	 * 
	 * @param aNode the TreeNode to get the path for
	 * @param depth an int giving the number of steps already taken towards the root
	 *              (on recursive calls), used to size the returned array
