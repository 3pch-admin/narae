...
public class WorkflowHelper2 implements RemoteAccess, Serializable {
	...
	public static QueryResult getWorkItemForChange(WTPrincipal principal) throws WTException {
		...
		cond = new SearchCondition(WorkItem.class, "ownership.owner.key.id", SearchCondition.EQUAL, Long.valueOf(userId));
		...
		cond = new SearchCondition(WorkItem.class, "primaryBusinessObject.key.classname", SearchCondition.LIKE, "%wt.change2.WTChangeRequest2%");
		spec.appendWhere(cond);
		spec.appendOr();
		cond = new SearchCondition(WorkItem.class, "primaryBusinessObject.key.classname", SearchCondition.LIKE, "%wt.change2.WTChangeOrder2%");
		spec.appendWhere(cond);
	...
	public static WorkItem getTargetWorkItem(WTPrincipal principal, LifeCycleManaged object, String wotkItemState, String roleName) throws WTException {
		QuerySpec spec = new QuerySpec(WorkItem.class);
		SearchCondition cond = null;
		...
		if(!RemoteMethodServer.ServerFlag) {
			...
			try {
		        ...
		        if( comment != null && comment.trim().length() > 0 ) {
		        	...
		        	WfEventHelper.createVotingEvent(null, 
...
//			SessionServerHelper.manager.setAccessEnforced(true);
	...
	public static QueryResult getVotingEventsForActivity(WfAssignedActivity activity, boolean isAll, boolean order) throws WTException, WTPropertyVetoException {
		...
		qs.appendWhere(new SearchCondition(new ClassAttribute(WfVotingEventAudit.class,"activityKey"), "=", new ClassAttribute(WfAssignedActivity.class, "key")), new int[]{votingIndex,activityIndex});
		...
		if( activity != null ) {
			...
			qs.appendWhere(new SearchCondition(WfActivity.class, WTAttributeNameIfc.ID_NAME, "=", id), new int[]{activityIndex});
		}
		qs.setQuerySet(false);
		OrderBy orderBy = new OrderBy(new ClassAttribute(WfVotingEventAudit.class, WTAttributeNameIfc.UPDATE_STAMP_NAME), order);
	...
	public static String getComment(WorkItem workItem) throws Exception {
		QueryResult result = WorkflowHelper2.getVotingEventsForActivity((WfAssignedActivity)workItem.getSource().getObject(), false, true);
		String comment = null;
		if( result.size() == 1 ) {
			WfVotingEventAudit voting = (WfVotingEventAudit)result.nextElement();

			comment = (voting.getUserComment() == null || voting.getUserComment().trim().equals(""))?"":voting.getUserComment();
		} else {
			comment = "";
		}
		return comment;
	...
	public static boolean changeState(WTObject pbo, String state) throws WTException {
		...
		if( pbo instanceof WTChangeOrder2 ) {
			...
			while( result.hasMoreElements() ) {
				...
				if( !part.getState().getState().toString().trim().equals("APPROVED") && !part.getState().getState().toString().trim().equals("SENT_ERP") ) { 
					log.debug("4. 부품번호/버젼......" + part.getNumber() + "/" + part.getVersionInfo().getIdentifier().getValue() + "." + part.getIterationInfo().getIdentifier().getValue());
					...
					if ( !VersionControlHelper.isLatestIteration(part) ) {
						...
						log.debug("4-3. 최신 이터레이션......" + part.getNumber() + "/" + part.getVersionInfo().getIdentifier().getValue() + "." + part.getIterationInfo().getIdentifier().getValue());
					}
					...
					if( epm3d != null) {
						log.debug("5-1. 해당 부품에 연결된 3D는....."+ epm3d.getNumber() + "/" + epm3d.getVersionInfo().getIdentifier().getValue() + "." + epm3d.getIterationInfo().getIdentifier().getValue());
						...
						if( epm2d != null ) {
							log.debug("6-1. 해당 3D에 연결된 2D는....."+ epm2d.getNumber() + "/" + epm2d.getVersionInfo().getIdentifier().getValue() + "." + epm2d.getIterationInfo().getIdentifier().getValue());
		...
		if( pbo instanceof WTChangeOrder2 ) {
			...
			while( result.hasMoreElements() ) {
				...
				if( epm3d != null) {
					...
					if( approvalLine.getChangeManagerInfo2() != null && approvalLine.getChangeManagerInfo2().size() > 0 ) {
...
//					AttributeHelper.service.setValue(epm3d, attribute);
					...
					if( epm2d != null ) {
						log.debug("6-1. 해당 3D에 연결된 2D는....."+ epm2d.getNumber() + "/" + epm2d.getVersionInfo().getIdentifier().getValue() + "." + epm2d.getIterationInfo().getIdentifier().getValue());
						log.debug("6-2. 해당 3D에 연결된 2D의 Publish 이벤트 생성......"+epm2d.getPersistInfo().getObjectIdentifier().toString());
						...
						//pub.doPublish(false, true, objRef, (ConfigSpec)null, (ConfigSpec)null, true, null, null, Publisher.EPM, null, 0);
